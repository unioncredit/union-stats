...
function useGovernanceStatsViewQuorum() {
  const { quorum } = useGovernanceStats();

  return [{ label: "Voting Quorum", value: unionValue(quorum) }];
}
...
function useGovernanceStatsViewVotingPeriod() {
  ...
  const { votingPeriod } = useGovernanceStats();
...
function useGovernanceStatsVotingDelay() {
  const { votingDelay } = useGovernanceStats();
...
function useGovernanceStatsViewTimelock() {
  ...
  const { timelock } = useGovernanceStats();
...
function useGovernanceStatsViewThreshold() {
  const { threshold } = useGovernanceStats();

  return [{ label: "Proposal Threshold", value: unionValue(threshold) }];
}
